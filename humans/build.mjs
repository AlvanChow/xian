...
if (!process.argv.includes('--check')) {
  ...
  /* The site module. src/humans.js is GENERATED from the same rows the standalone
     page uses, so the census on valuegrid and the census in humans/ can never
     disagree. Field names keep the H- prefix to sit alongside COMPANIES/FLOWS and
     RENTS in the existing namespace. */
  const mod = `/* GENERATED by humans/build.mjs — do not edit.
   The $${''}{rows.length}-person US $100M-$5B census. Regenerate with: node humans/build.mjs
...
export const HUMANS = ${JSON.stringify(rows)};
`;
  writeFileSync(join(HERE, '..', 'src', 'humans.js'), mod);
  console.log(`wrote src/humans.js (${(mod.length / 1024).toFixed(0)} KB, ${rows.length} rows)`);
