...
if (!process.argv.includes('--check')) {
  ...
  /* The site pieces. The metadata module is tiny and ships inside the single-file
     build; the 557 records are written separately to src/public/ so vite copies
     them alongside dist/index.html and the census fetches them only when its tab
     is first opened. That keeps the Map and the Scarcity board — the site's front
     door — at their original weight instead of paying for a dataset most visitors
     never open. The cost is that the census, unlike the rest of the site, needs
     index.html to be served rather than opened over file://; it says so on screen
     when that fetch fails. */
  const meta = `/* GENERATED by humans/build.mjs — do not edit.
...
export const HASOF = ${JSON.stringify(ASOF)};
export const HCOUNT = ${rows.length};
export const HBANDS = ${JSON.stringify(BANDS)};
export const HAGEBANDS = ${JSON.stringify(AGEBANDS)};
export const HREGIONS = ${JSON.stringify(REGIONS)};
export const HSECTORS = ${JSON.stringify(SECTORS)};
...
`;
  writeFileSync(join(HERE, '..', 'src', 'humans.js'), meta);
  const pub = join(HERE, '..', 'src', 'public');
  mkdirSync(pub, { recursive: true });
  writeFileSync(join(pub, 'humans-data.json'), JSON.stringify({ asof: ASOF, rows }));
  console.log(`wrote src/humans.js (${(meta.length / 1024).toFixed(0)} KB meta)`
    + ` + src/public/humans-data.json (${(JSON.stringify({ asof: ASOF, rows }).length / 1024).toFixed(0)} KB, ${rows.length} rows)`);
